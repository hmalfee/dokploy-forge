...
interface TemplateData {
  ...
  config?: {
    ...
    env?:
      ...
      | Array<string | Record<string, string | boolean | number>>;
    mounts?: MountConfig[];
  };
}
...
class TemplateValidator {
  ...
  private parseComposeServices(composePath: string): string[] {
    try {
      ...
      if (serviceNames.length === 0) {
        this.warning(
          `No services found in docker-compose.yml at ${composePath}`,
        );
      }
  ...
  private validateTemplate(
    tomlPath: string,
    composeServices: string[] | null = null,
  ): boolean {
    try {
      ...
      try {
        ...
        this.error(`Invalid TOML syntax in ${tomlPath}: ${parseError.message}`);
        return false;
      }
      ...
      if (data.config.domains) {
        ...
        data.config.domains.forEach((domain, index) => {
          ...
          if (!domain.serviceName) {
            this.error(
              `domain[${index}]: Missing required field 'serviceName'`,
            );
          }
      ...
      if (data.config.env !== undefined) {
        if (Array.isArray(data.config.env)) {
          ...
          typeof data.config.env === "object" &&
          data.config.env !== null
        ) {
      ...
      if (data.config.mounts) {
        if (!Array.isArray(data.config.mounts)) {
          ...
          data.config.mounts.forEach((mount, index) => {
            if (!mount.filePath) {
              this.error(
                `config.mounts[${index}]: Missing required field 'filePath'`,
              );
            ...
            if (mount.content === undefined) {
              this.error(
                `config.mounts[${index}]: Missing required field 'content'`,
              );
          ...
          try {
            ...
            if (data.config.domains) {
              data.config.domains.forEach((domain, index) => {
                if (domain.host && typeof domain.host === "string") {
                  try {
                    const processedHost = processValue(
                      domain.host,
                      processedVars,
                      schema,
                    );
            ...
            if (this.options.verbose) {
              ...
              this.log(
                `📋 Processed ${Object.keys(processedVars).length} variables`,
                "debug",
              );
            }
          } catch (e: any) {
