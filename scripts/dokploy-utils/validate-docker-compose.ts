...
class DockerComposeValidator {
   ...
   */
  private validateNoContainerName(
  ...
  private validateNoExplicitNetworks(
    ...
    const hasDokployNetwork =
      compose.networks && "dokploy-network" in compose.networks;
  ...
  private validateServicesExist(compose: ComposeSpecification): boolean {
    ...
    this.log(
      `Found ${serviceNames.length} service(s): ${serviceNames.join(", ")}`,
      "debug",
    );
    ...
    if (this.errors.length === 0) {
      ...
      if (this.options.verbose) {
        ...
        Object.keys(services).forEach((serviceName) => {
          ...
          const image =
            typeof service.image === "string" ? service.image : "N/A";
